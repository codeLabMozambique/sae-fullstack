...
const Biblioteca: React.FC = () => {
  ...
  // Carrega conteúdos sempre que muda a disciplina seleccionada
  useEffect(() => {
    setLoading(true);
    setError(null);
    const params = activeDiscipline === ALL_LABEL ? {} : { discipline: activeDiscipline };
    listContents({ ...params, size: 50 })
  ...
  const handleRead = (book: Content) => {
    window.open(readUrl(book.id), '_blank');
  };

  const handleDownload = (book: Content) => {
    const url = readUrl(book.id);
      ...
      <Box sx={{
        ...
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {loading ? 'A carregar…' : `${books.length} títulos disponíveis`}
          </Typography>
        </Box>
        <TextField
          size="small"
            ...
            <Grid key={book.id} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
              <Card sx={{
                ...
                <CardActions sx={{ px: 2, pb: 2, pt: 1, gap: 1 }}>
                  ...
                  <Tooltip title="Descarregar PDF">
                    <IconButton size="small" onClick={() => handleDownload(book)} sx={{
                      border: '1px solid #E5E7EB', borderRadius: 1.5,
